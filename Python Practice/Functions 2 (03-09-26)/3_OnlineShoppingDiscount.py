...
prices = [int(x) for x in input(f"Enter {n} prices : ").split()]

eligible = list(filter(lambda x:x>=2000,prices))

discounted = list(map(lambda x:x-x*0.2,eligible))

print("Discounted Prices:",*discounted,sep=",")